 ...
 * @param $atts
 */
function shortcode_new_naver_sign_button() {
    echo  new_naver_sign_button(true);
}
...
function new_naver_sign_button($custom = false) {

  global $new_naver_settings;
  if($custom){
	return '<a href="' . new_naver_login_url() . (isset($_GET['redirect_to']) ? '&redirect=' . $_GET['redirect_to'] : '') . '"><i class="xi-naver"></i><span>네이버 아이디로<br/>로그인</span></a>';
  } else {
...
function new_naver_login_url() {
  $naver_connect = maybe_unserialize(get_option('naver_connect'));
  return $naver_connect['naver_redirect'] . '?loginNaver=1';
}
